...
class ElevenLabsText2SpeechTool(BaseTool):  # type: ignore[override]
    "Langchain class overriden because of a persistent error in the original class"
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        values["elevenlabs_api_key"] = get_from_dict_or_env(
    ...
    def _run(
        ...
        client = elevenlabs.client.ElevenLabs(api_key = self.elevenlabs_api_key)
        try:
            ...
            if hasattr(speech, '__iter__') and not isinstance(speech, (bytes, bytearray)):
                speech_bytes = b''.join(chunk for chunk in speech)
            else:
                speech_bytes = speech
